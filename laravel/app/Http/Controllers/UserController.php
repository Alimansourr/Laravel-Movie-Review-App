  ...
  public function addreview(Request $request)
{
    ...
    $movieId = $request->input('movie_id'); // ✅ get from form, not session
    ...
    return redirect("/searching/{$movieId}");
}
...
{
    // however you already fetch the movie & reviews:
    $movie   = \App\Models\Movie::findOrFail($id);
    $reviews = \App\Models\Review::where('movie_id', $id)->get();

    // compute average rating (rounded to 1 decimal) or null if no reviews
...
public function profile()
{
    ...
    $user = $useman->getUserByEmail($userEmail); // 👈 we'll define this in UserManager
